...
DEFAULT_SEED_LIST = [i for i in range(1, 25)]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run deterministic heuristics, Goldberg-Liu, approximation, and NaDS for the technology diffusion experiment."
        )
    )
    ...
    parser.add_argument("--min-conn", type=int, default=10)
    parser.add_argument("--mg-max-depth", type=int, default=3)
...
def main() -> None:
    ...
    try:
        for run_idx, (n_nodes, c, seed) in enumerate(combinations, start=1):
            ...
            approx_k, final_x, approx_runtime, approx_history = approx(g, thetas, max_time=max_time)
            append_result(
                results,
                "approx",
                n_nodes,
                ...
                [list(event) for event in approx_history] if approx_history is not None else None,
            )
            print_algorithm_line(
                "approx",
